# QuestWordHunter — German Voiceover: Legion

The German a quest giver says out loud, for Legion: quest ids
**39,695–48,158**.

**2,402 clips, 2.8 hours.** Both are counted from this pack's own duration
...
`sounds/` is gitignored, and `.gitignore` says why: it is 50 MB here and around
seven gigabytes across the twelve packs, and how that should ship has not been
decided. So a checkout of this repository is not installable by itself — it is
the manifest, the licence and the duration table, and no sound.

The playable pack is assembled by `Tools/build_pack.py` in the engine
repository, which takes these files and adds the clips:

```
python Tools/build_pack.py --only Legion --out "…/Interface/AddOns"
```
